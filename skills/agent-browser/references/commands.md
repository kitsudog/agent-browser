...
agent-browser wait --download [path]       # Wait for a download to complete
```

Default timeout is 25 seconds. To raise it, set the
`AGENT_BROWSER_DEFAULT_TIMEOUT` env var (no per-command `--timeout` flag on
`wait --url`/`wait --text`).

## Downloads

```bash
agent-browser download @e1 ./report.pdf    # Click element to trigger download, save to path
agent-browser wait --download              # Wait for any download to complete
agent-browser wait --download ./out.csv    # Wait + save to a specific path
```

Use `--download-path <dir>` (or the `AGENT_BROWSER_DOWNLOAD_PATH` env var) to
set a default download directory. Without it, downloads go to a temporary
directory that is deleted when the browser closes.
...
agent-browser set media light                 # Emulate light color scheme
```

For a color scheme that persists across all commands, use the `--color-scheme`
global flag instead:
