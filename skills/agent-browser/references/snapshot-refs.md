...
For complex pages, scope the snapshot to a CSS selector:
...
Refs are outputs of a snapshot, not inputs — scope with `-s "<css-selector>"`,
not with a `@eN` ref.
...
```bash
# Snapshot a specific container with a CSS selector
